...
class ShowContactDetailsController @Inject() (
  ...
  def show(): Action[AnyContent] = identifier andThen resolveSessionId async { implicit request =>
    accountLinkCacheService.get(request.request.user.internalId).flatMap {
      ...
      case Some(details) =>
        ...
        } yield Ok(view(viewModel, details.statusId, details.linkId, details.isNiAccount))).recover {
          case NonFatal(e) =>
            log.error(s"Unable to retrieve account details: ${e.getMessage}")
            InternalServerError(
              errorView(details.dan, Option(appConfig.financialsHomepage), details.statusId, details.linkId)
            )
        }
    }
  }
