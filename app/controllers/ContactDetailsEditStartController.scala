        ...
        initialContactDetails <-
          liftF(
            contactDetailsCacheService
              .getContactDetails(request.request.user.internalId, dutyDefermentDetails.dan, dutyDefermentDetails.eori)
          )
        initialUserAnswers    <- fromOption(
                                   setUserAnswers(
                                     initialContactDetails,
                                     dutyDefermentDetails,
    ...
    contactDetailsChange match {
      case true =>
        UserAnswers(internalId, lastUpdated = dateTimeService.now())
          .set(EditContactDetailsPage, initialUserAnswers)
          .toOption
      case _    =>
        UserAnswers(internalId, lastUpdated = dateTimeService.now())
          .set(EditAddressDetailsPage, initialAddressUserAnswers)
          .toOption
    }
  }
}
