...
class AccountLinkCacheServiceSpec extends SpecBase {
    ...
    "return AccountLink on successful submission" in new Setup {
      when(mockSessionCacheConnector.retrieveSession(any, any)(any))
        .thenReturn(Future.successful(Some(AccountLink(
          "someEori", "dan", "linkId", AccountStatusOpen, Some(DefermentAccountAvailable)))))
  ...
  trait Setup {
    ...
    val dutyDefermentAccountLink: DutyDefermentAccountLink = DutyDefermentAccountLink("someEori",
      "dan", "someLinkId", AccountStatusOpen, DefermentAccountAvailable)
