...
class HmrcMongoSessionCacheSpec extends SpecBase with OptionValues with ScalaFutures with IntegrationPatience {

  private val id = "session-123"
  private val test: Test = Test("john doe", 123)
  private val app = application().build()
  private val legacyCache = app.injector.instanceOf[TestCache]
  private val testCache = app.injector.instanceOf[HmrcMongoTestCache]
  ...
  "Session Cache" must {
    ...
    "retrieve values set by the legacy cache" in {
      testCache.retrieve(id).futureValue mustBe None
      legacyCache.store(id, test).futureValue
      testCache.retrieve(id).futureValue.value mustBe test
    }
  }
}